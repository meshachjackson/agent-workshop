...
The repo is the source of truth for both workflows, not the n8n UI. Editing in n8n and forgetting to export is how the exports came to sit ten times behind the running workflows, with three byte-identical copies of the QA aggregation block drifting free. Pull with `scripts/normalize-workflow.mjs`, which produces a deterministic export (slug ids from node names, n8n's duplicate "1" suffixes stripped along with the `$('Node1')` references that follow them, credentials removed, keys sorted) so `git diff` can prove the repo and the live workflow agree. The QA sub-workflow's two code nodes are generated from `n8n/qa-prepare.js` and `n8n/qa-aggregation.js`; edit those files, not the JSON. `tests/workflow.test.ts` enforces all of it, including per-node and per-workflow size ceilings, which exist so a workflow always stays small enough to read in one call.
